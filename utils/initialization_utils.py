...
from .latent_utils import get_delta_weight, forward_edora
...
def find_and_initialize(model, peft_config, adapter_name, reconstr_type, reconstruct_config, writer, sigma_val = 0.00001):
    """
        ...
        if target_module_found:
            ...
            if reconstruction_mode == 'separated':
                ...
                if not isinstance(target, peft.tuners.lora.Linear):
                    ...
                    if r_squared:
                        target.forward = types.MethodType(forward_edora, target)
                        ...
                        init_module_weights(target.default_lora_latent_mapping, sigma=sigma_val)
                        ...
                        init_module_weights(target.lora_A.default, sigma=sigma_val)
